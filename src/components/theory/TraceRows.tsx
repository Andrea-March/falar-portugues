...
export interface TraceRow {
  key: string;
  /** Testo sopra il campo: il pronome ("Eu") o la situazione ("☀️ Sono le 9…") */
  label: string;
  /** Cosa si digita: solo lettere, spazi e accenti */
  form: string;
  /** Punteggiatura finale, mostrata ma non digitata (es. "?" in "Tudo bem?") */
  after?: string;
  /** Cosa si legge ad alta voce a riga completata */
  spoken: string;
}

interface TraceRowsProps {
  rows: TraceRow[];
  /** trace = si ricopia sopra il modello in trasparenza; recall = si scrive a memoria */
  mode: 'trace' | 'recall';
  /** Schermata già completata (si è tornati indietro): tutto compilato */
  initiallyDone: boolean;
  onDone: () => void;
  /** Prefisso per gli id dei campi (unico nella pagina) */
  idPrefix: string;
  /** Etichetta grande (situazioni) invece della piccola (pronomi) */
  longLabels?: boolean;
}

/**
 * Righe da completare, una alla volta.
 * - trace: il modello è in trasparenza e si ricopia; le lettere sbagliate non entrano,
 ...
 *   Se si scrive la risposta di un'altra riga, lo si fa notare con garbo.
 */
export default function TraceRows({ rows, mode, initiallyDone, onDone, idPrefix, longLabels = false }: TraceRowsProps) {
  const trace = mode === 'trace';
  const [typed, setTyped] = useState<string[]>(() => rows.map((r) => (initiallyDone ? r.form : '')));
  const [status, setStatus] = useState<RowStatus[]>(() => rows.map(() => (initiallyDone ? 'done' : 'open')));
  ...
  const accentChars = [...new Set(rows.flatMap((r) => chars(r.form)).filter((c) => base(c) !== c.toLowerCase()))];
  ...
  const traceInput = (i: number, raw: string) => {
    const r = applyTrace(typed[i], raw, rows[i].form);
  ...
  const recallInput = (i: number, raw: string) => {
    ...
    setRowHint(null);
    if (matchAnswer(raw, rows[i].form) === 'exact') finishRow(i, 'done', next.map((t, j) => (j === i ? rows[i].form : t)));
    else setTyped(next);
  };
  ...
  const reveal = (i: number) => {
    soundFX.playClick();
    finishRow(i, 'revealed', typed.map((t, j) => (j === i ? rows[i].form : t)));
  };
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <label htmlFor={inputId} className={longLabels ? 'font-bold text-ink/80 leading-snug' : 'font-bold text-brand-muted'}>
                  {row.label}
                </label>
                {st !== 'open' ? (
                  <button
                  ...
                  !trace && (
                    <button
                      type="button"
                      onClick={() => reveal(i)}
                      className="inline-flex items-center gap-1 text-sm font-bold text-brand-muted hover:text-azulejo-dark cursor-pointer shrink-0"
                    >
              ...
              <div className="relative mt-1.5 flex items-center gap-2">
                {/* Livello visivo: lettere scritte piene, modello in trasparenza, punteggiatura fissa */}
                <div aria-hidden="true" className={`${wordClass} py-1 whitespace-pre`}>
                  {st !== 'open' ? (
                    <span className={st === 'done' ? 'text-ok-dark' : 'text-azulejo-dark'}>
                      {row.form}
                      {row.after}
                    </span>
                  ) : trace ? (
                    <>
                      ...
                      <span className="text-ink/40">{row.after}</span>
                    </>
                  ) : (
                    <>
                      <span className="text-ink">{typed[i]}</span>
                      <span className="text-ink/30">{typed[i] ? row.after : '\u00a0'}</span>
                    </>
                  )}
                </div>
